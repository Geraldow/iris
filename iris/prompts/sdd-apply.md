...
<!-- Purpose: Implement tasks from the change, writing actual code following the specs and design -->
...
4. **Execute Phase Objective**: 
   - Write and modify source code to fulfill the required tasks.
   - Mark off completed tasks and ensure the implementation matches the specifications and design.
5. **Validation**: Check your code against `{instruction}` and run basic checks. Ensure it meets the standard for `{phase}`.
6. **Save to Engram**: Save your execution summary and code patches to Engram. You MUST use exactly this topic key: `sdd/{change}/{phase}`. Include standard type and project fields.
