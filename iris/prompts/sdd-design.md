...
<!-- Purpose: Create technical design document with architecture decisions and approach -->
...
4. **Execute Phase Objective**: 
   - Detail architectural decisions and applied patterns.
   - Define data models, APIs, and component interactions required for the change.
5. **Validation**: Check your design against `{instruction}` and the specs. Ensure it meets the standard for `{phase}`.
6. **Save to Engram**: Save your finalized design to Engram. You MUST use exactly this topic key: `sdd/{change}/{phase}`. Include standard type and project fields.
