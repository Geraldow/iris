...
<!-- Purpose: Validate that implementation matches specs, design, and tasks -->
...
4. **Execute Phase Objective**: 
   - Run necessary tests and static analysis.
   - Verify the implementation strictly against the specs and report any discrepancies or bugs.
5. **Validation**: Check your verification findings against `{instruction}`. Ensure it meets the standard for `{phase}`.
6. **Save to Engram**: Save your finalized verification report to Engram. You MUST use exactly this topic key: `sdd/{change}/{phase}`. Include standard type and project fields.
