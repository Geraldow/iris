...
4. **Execute Phase Objective**: 
   - Define the intent and precise scope of the change.
   - Outline the high-level approach and key milestones.
5. **Validation**: Check your proposal against `{instruction}`. Ensure it meets the standard for `{phase}`.
6. **Save to Engram**: Save your finalized proposal to Engram. You MUST use exactly this topic key: `sdd/{change}/{phase}`. Include standard type and project fields.
