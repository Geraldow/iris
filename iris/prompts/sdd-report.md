...
<!-- Purpose: Generate a summary report of the completed change -->
...
4. **Execute Phase Objective**: 
   - Summarize the entire lifecycle of the change and the finalized state.
   - Document any lessons learned, technical debt incurred, and deployment instructions.
5. **Validation**: Check your report against `{instruction}`. Ensure it meets the standard for `{phase}`.
6. **Save to Engram**: Save your finalized report to Engram. You MUST use exactly this topic key: `sdd/{change}/{phase}`. Include standard type and project fields.
